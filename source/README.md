...
## Running repofortestingm2k in Quarkus dev mode
...
```
(repofortestingm2k should be included in the returned array)
...
[repofortestingm2k-schema.json](./src/main/resources/schemas/repofortestingm2k-schema.json ), run the following to create an
instance of the repofortestingm2k workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/repofortestingm2k -d@input.json
```
