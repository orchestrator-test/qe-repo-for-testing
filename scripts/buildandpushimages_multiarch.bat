...
echo "building and pushing image repofortestingm2k-buildstage"
pushd source
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k-buildstage . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k-buildstage
    podman build --platform ${PLATFORMS} -f Dockerfile.buildstage --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k-buildstage .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k-buildstage
)
popd
echo "building and pushing image repofortestingm2k"
pushd source
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k
    podman build --platform ${PLATFORMS} -f Dockerfile --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/repofortestingm2k
)
popd
